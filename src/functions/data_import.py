...
S3_PREFIX_IMP_BEF = "image-file/"
S3_PREFIX_IMP_AFT = "image-file-done/"

def dataImport(event, context):
    s3_client = boto3.client('s3')
    s3_file_lists = s3_client.list_objects_v2(Bucket=BUCKET_NAME, Prefix=S3_PREFIX_IMP_BEF)
    # フィールド定義
    fieldnames = ("id","title","descriptive_text","coupon_url","coupon_img_url")
    ...
    for filename in s3_file_lists['Contents']:
        if 0 != filename['Size']:
            ...
            for row in csv.DictReader(lines,fieldnames):
                jsonList.append(row)
    # レコードを追加
    for putRow in jsonList:
        DYNAMODB_TABLE_INFO.put_item(Item=putRow)
    return 'Done'
